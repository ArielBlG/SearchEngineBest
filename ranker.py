...
class Ranker:
    ...
    def get_matrix(relevant_doc, doc_set = None):
        ...
        for key in relevant_doc.keys():
            ...
            res_dic[key] = dict(map(lambda item: (item[0], item[1]*np.log2(num_of_docs/len(relevant_doc[key]))),relevant_doc[key]))
    ...
    def rank_relevant_doc(relevant_doc, doc_set = None, vector=None, tweets_dict = None):
        ...
        start = time.time()
        out = Ranker.cos_sim(matrix=matrix_wo_id, vector=vector, tweets_w_ij=w_i_j_vector) #.reshape((1,vector.shape[0]))
        end = time.time()
